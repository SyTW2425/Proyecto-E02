...
describe('POST /api/cards/:name', () => {
  ...
  it('debería devolver un error si el usuario no existe', async () => {
    ...
    const response = await request(app)
      .post('/api/cards/nonexistentuser')
      .send(newCard)
      .expect(404);
...
describe('GET /api/cards', () => {
  ...
  it('debería obtener una carta por su ID', async () => {
    ...
    const response = await request(app).get(`/api/cards/${card._id}`).expect(200);
  ...
  it('debería devolver un error si la carta no existe', async () => {
    ...
    const response = await request(app).get(`/api/cards/${nonExistentId}`).expect(404);
