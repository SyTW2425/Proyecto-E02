...
describe('PUT /cards/:id', () => {
  ...
  it('debería actualizar una carta existente', async () => {
    ...
    expect(response.body).to.have.property('msg', 'Carta actualizada con éxito');
    expect(response.body.Card).to.include({ name: 'Updated Charmander', hp: 60 });
  });
  it('debería devolver un error si la carta no existe', async () => {
    const response = await request(app)
      .put('/cards/invalidcard')
      .send({ name: 'Updated Charmander' })
      .expect(500);
