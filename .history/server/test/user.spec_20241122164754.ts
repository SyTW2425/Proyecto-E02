...
describe('GET /users', () => {
  ...
  beforeEach(async () => {
    ...
    await User.insertMany([
      { name: 'user1', email: 'user1@example.com', password: 'Str0ngP@ssw0rd!123' },
      { name: 'user2', email: 'user2@example.com', password: 'Str0ngP@ssw0rd!123' },
    ]);
  });
...
describe('GET /users/:id', () => {
  ...
  beforeEach(async () => {
    await User.deleteMany({});
    testUser = new User({ name: 'user1', email: 'user1@example.com', password: 'Str0ngP@ssw0rd!123' });
    await testUser.save();
  });
...
describe('PUT /users/:id', () => {
  ...
  it('debería actualizar un usuario existente', async () => {
    ...
    expect(response.body).to.have.property('msg', 'Se ha actualizado correctamente el usuario');
    expect(response.body).to.have.property('User');
    expect(response.body.User).to.include({
      name: 'updatedUser',
      email: 'updateduser@example.com',
...
describe('GET /api/users/:id/cards', () => {
  let testUser: InstanceType<typeof User>;
  ...
  it('debería obtener las cartas de un usuario existente', async () => {
    const response = await request(app)
      .get(`/api/users/${testUser._id}/cards`)
      .expect(200);
  ...
  it('debería devolver un error si el usuario no existe', async () => {
    ...
    const response = await request(app)
      .get(`/api/users/${nonExistentId}/cards`)
      .expect(404);
