    ...
    if (req.body) {
      ...
      try {
        ...
        if (catalogs) {
          ...
          res.status(404).send({ error: 'Catalogs not found' });
        }
      } catch (error) {
        res.status(500).send(error);
      }
 ...
 */
catalogRouter.get('/catalogs/search', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (cards.length === 0) {
      res.status(404).json({ msg: 'No se encontraron cartas con los filtros proporcionados' });
    }
 ...
 */
catalogRouter.get('/catalogs/:catalogId/cards', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    res.status(500).json({ msg: 'Error al buscar las cartas del catálogo', error });
  }
});